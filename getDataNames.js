...
async function getDataNames(req, res) {
    ...
    const data = await dataTahunAjaran.findOne({ tahun_ajaran : tahunajaran})
    ...
    if (data) {
        ...
        if (isKbmAktif) {
            ...
            res.status(200).send({ messege : "kbm tidak aktif !"});

        };

    } else {

        res.status(200).send({ messege : "data tahun ajaran tidak detemukan !"})
    ...
    function checkIsKBMAcktive(data) {
        ...
        for (let i = 0; i < kbmNonAktif.length; i++) {
             ...
             */
            const isFirst = (tanggalAwal - tanggal) == 0 && (tanggalAkhir - tanggal) != 0 && jamAwal <= jam;
            const isLast = (tanggalAwal - tanggal) != 0 && (tanggalAkhir - tanggal) == 0 && jamAkhir >= jam;
            const isBetwen = (tanggalAwal - tanggal) < 0 && (tanggalAkhir - tanggal) > 0;
            const isNow = (tanggalAwal - tanggal) == 0 && (tanggalAkhir - tanggal) == 0 && (jamAwal == jam || jamAkhir == jam)

            let isTarget;
            if(targets && targets.length && Array.isArray(targets)){
                isTarget = targets.filter(isTarget = target => target == parseInt(idkelas))
            } else {
                isTarget = [];
            }
    ...
    async function foundedData() {
        ...
        const data = await dataSantri.find({
        ...
        }).catch((err) => {return { status : 500, messege : "internal server error", error : err}});
        ...
        if (data.length !== 0) {
            ...
            data.forEach(element => {
                ...
                const entry = element.statistik_madrasah_diniyyah.find(entry => entry.idkelas === idkelas);
                const nameData = {
                    id : element.id,
        ...
        return { status : 200, data : responseData};
            ...
            return { status : 200, messege : "data tidak detemukan !"}
        };
    };
};
